...
def sort_faces_to_canonical(faces, in_place=False, device='cuda'):
    ...
    mask = max_index == 1
    faces_opp_orient = torch.index_select(faces, 1, torch.tensor([0, 2, 1], device=device))
...
# It is required that the four points of the quadrilateral should be coplanar as much as possible
def sample_points_on_surface(verts, faces, n_pts, return_inds_and_bary=False, face_probs=None):
    ...
    bary_vals[:, 2] = r1_sqrt * r2 * (1. - r3)
    bary_vals[:, 3] = r1_sqrt * r2 * r3
...
#   faces: (F, 4) inds
#   return: (N, F)
def point_quadrilateral_distance(points, verts, faces):
    n_p = points.shape[0]
    n_f = faces.shape[0]
    ...
    face_normals = face_normals(verts, faces)
...
#  candidate_probs (B, C)
def uniqueify_triangle_prob_batch(candidate_triangles, candidate_probs, debug_checks=False):
...
# Input:
#  candidate_triangles (B, C, 3)
def uniqueify_triangle_batch(candidate_triangles, debug_checks=False):
    B = candidate_triangles.shape[0]
    C = candidate_triangles.shape[1]
