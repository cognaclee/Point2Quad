...
from metrics import mesh_utils

import torch_scatter


'''
face loss function:Reduce holes and overlapping faces
input:
  pred_probs:candidate face Log probabilities belonging to class 0 and class 1 respectively, n*2
  labels: candidate face label, n*1
  
function:
  loss = pred_probs[:,0]*labels+pred_probs[:,1]*(1-labels)

  distance = 1-labels : Indicative distance function, candidate face in gt is 0, not in gt is 1
  
'''

def surface_loss(pred_probs, labels, device="cuda:0"):
    
    dist_indicate = torch.zeros((labels.shape[0],2)).to(device)
    dist_indicate[:,0] = labels
    dist_indicate[:,1] = 1-labels
    dist_indicate = dist_indicate.detach()
    #print('pred_probs.shape=',pred_probs.shape)
    #print('dist_indicate.shape=',dist_indicate.shape)
    loss = torch.mean(torch.exp(pred_probs)*dist_indicate)
    return loss


# Distance from a known surface to a generated quadrilateral with probabilities
def dist_surface_to_quadrilateral_probs(surf_verts, cand_faces, label, pred, pred_face_probs, surf_samples):
    if torch.sum(pred) == 0:
    ...
    char_len = norm(surf_samples - torch.mean(surf_samples , dim=0, keepdim=True)).mean()

    # Find the distance to all triangles in the generated surface
    gen_faces = cand_faces[pred==1]
    gen_face_probs = pred_face_probs[pred==1]
    quad_dists = mesh_utils.point_quadrilateral_distance(surf_samples, surf_verts, gen_faces)

    # Sort distances
    k_val = min(32, quad_dists.shape[-1])
    quad_dists_sorted, sorted_inds = torch.topk(quad_dists, largest=False, k=k_val, dim=-1)
    ...
    last_dist = char_len * torch.ones(quad_dists.shape[0], dtype=quad_dists.dtype, device=quad_dists.device)
    ...
    tri_dists_sorted = torch.cat((quad_dists_sorted, last_dist.unsqueeze(-1)), dim=-1)

    
    
    # Use these likelihoods to get expected distance
    expected_dist = torch.sum(prob_is_closest * quad_dists_sorted, dim=-1)
...
# Distance from generated quadrilateral with probabilities to a known surface
def dist_quadrilateral_probs_to_sampled_surface(surf_pos, surf_normals, gen_verts, gen_faces, gen_face_probs, n_sample_pts=5000):
    ...
    # Sample points on the generated quadrilateral
    samples, face_inds, _ = mesh_utils.sample_points_on_surface(gen_verts, gen_faces, n_sample_pts, return_inds_and_bary=True)
...
# Penalize overcomplete quadrilateral which overlap themselves by evaluating a spatial kernel at sampled surface
