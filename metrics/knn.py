...
def all_face_neighbors(faces, n_target, debug_checks=False,device='cuda'):
    n_face = faces.shape[0]

    if debug_checks:
        check_faces_for_duplicates(faces)

    neighbors = torch.arange(n_target, device=device).unsqueeze(0).expand((n_face, -1))
